  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        {initialDocuments.map((section, si) => (
          <div key={si}>
            {/* Section label */}
              ...
              {section.items.map((doc, di) => (
                ...
                >
                  ...
                  <div className="flex items-start sm:items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-1">
                        ...
                        <span className="text-gray-300">•</span>
                        <span className="text-xs font-medium text-gray-400 whitespace-nowrap">{doc.type}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className={`flex items-center justify-center gap-2 text-sm px-4 py-2 rounded-lg transition w-full sm:w-auto flex-shrink-0 ${
                      doc.available
                        ? 'border border-gray-200 text-gray-600 hover:bg-gray-100'
                        : 'text-gray-300 border border-gray-100 bg-gray-50/50 cursor-not-allowed'
                    }`}
                  >
                    ...
                    <span className="whitespace-nowrap">{doc.available ? 'Download' : 'Not available'}</span>
                  </button>
                </div>
              ))}
            </div>
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 bg-gray-900 text-white text-sm px-5 py-3 rounded-xl shadow-lg flex items-center gap-3 z-50 animate-fade-in">
          ...
          <span className="pr-1">Document download will be available when backend is connected.</span>
        </div>
      )}
    </div>
  );
