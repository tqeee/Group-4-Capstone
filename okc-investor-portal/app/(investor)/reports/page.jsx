      ...
      {activeTab === 'Overview' && (
        <div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
            ...
            <div className="card p-6">
              <p className="section-label mb-4">FUND DETAILS</p>
            ...
            <table className="w-full min-w-[800px]">
                ...
                {monthlyData.map((row, i) => {
                  ...
                  return (
                    <tr key={i} className="hover:bg-gray-50/50 transition">
                      <td className="table-cell font-semibold text-gray-900">{row.month}</td>
                      <td className="table-cell text-gray-600">{row.tradingDays}</td>
                      <td className="table-cell text-gray-600">{formatCurrency(row.startValue, false)}</td>
                      <td className="table-cell text-gray-600">{formatCurrency(row.endValue, false)}</td>
                      <td className={`table-cell font-semibold ${row.totalPnl >= 0 ? 'text-green-600' : 'text-red-500'}`}>
                        {formatCurrency(row.totalPnl)}
                      </td>
                      <td className={`table-cell font-semibold ${row.return >= 0 ? 'text-green-600' : 'text-red-500'}`}>
                        {formatPercent(row.return)}
                      </td>
                      <td className="table-cell text-gray-600">{row.trades}</td>
              ...
              <tfoot>
                <tr className="border-t border-gray-200 bg-gray-50 font-semibold">
                  ...
                  <td className="table-cell text-gray-700">16</td>
                  <td className="table-cell text-gray-700">{formatCurrency(50000, false)}</td>
                  <td className="table-cell text-gray-700">{formatCurrency(34061.15, false)}</td>
                  <td className="table-cell font-bold text-red-500">{formatCurrency(-15938.85)}</td>
                  <td className="table-cell font-bold text-red-500">{formatPercent(-31.88)}</td>
                  <td className="table-cell text-gray-700">36</td>
                  <td className="table-cell text-gray-700">33.3%</td>
                </tr>
              </tfoot>
            </table>
          </div>
            ...
            <table className="w-full min-w-[650px]">
              ...
              <tbody className="divide-y divide-gray-50">
                {dailyData.map((row, i) => {
                  ...
                  return (
                    <tr key={i} className="hover:bg-gray-50/50 transition">
                      <td className="table-cell text-gray-600">{row.date}</td>
                      ...
                      <td className={`table-cell font-medium ${changePct >= 0 ? 'text-green-600' : 'text-red-500'}`}>
