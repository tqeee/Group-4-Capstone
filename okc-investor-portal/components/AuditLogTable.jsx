...
const filters = ['All', 'Login', 'Failed', 'MFA', 'Users', 'Transactions', 'Imports'];

const FILTER_MATCH = {
  Login: log => log.action.startsWith('LOGIN') || log.action === 'LOGOUT',
  Failed: log => !log.success,
  MFA: log => log.action.startsWith('MFA'),
  Users: log => log.action.startsWith('USER') || log.action.startsWith('PASSWORD'),
  Transactions: log => log.action.startsWith('FLOW'),
  Imports: log => log.action.startsWith('DATA') || log.action.startsWith('STATEMENT'),
};
...
// Shared audit trail table for admin (audit-logs) and operations (operation-log).
export default function AuditLogTable({ logs }) {
  const [search, setSearch] = useState('');
  const [activeFilter, setActiveFilter] = useState('All');

  const filtered = logs.filter(log => {
    const haystack = `${log.actorEmail ?? ''} ${log.action} ${log.detail ?? ''} ${log.ip ?? ''}`.toLowerCase();
    const matchSearch = haystack.includes(search.toLowerCase());
    const matchFilter = activeFilter === 'All' ? true : FILTER_MATCH[activeFilter]?.(log) ?? true;
    ...
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: 'FAILED / SUSPICIOUS', value: failCount, red: failCount > 0 },
          { label: 'UNIQUE USERS', value: uniqueActors, red: false },
        ].map((s, i) => (
          ...
          <table className="w-full min-w-[850px]">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                {['STATUS', 'USER', 'ACTION', 'DETAIL', 'IP ADDRESS', 'TIMESTAMP'].map(h => (
              ...
              {filtered.map(log => (
                <tr key={log.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-4 min-w-[180px]">
                    <p className="text-sm font-semibold text-gray-900 truncate">{log.actorEmail ?? '(unknown)'}</p>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`text-xs font-mono font-medium px-2 py-1 rounded ${log.success ? 'bg-gray-100 text-gray-600' : 'bg-red-50 text-red-600'}`}>
                      {log.action}
                    </span>
                  </td>
                  <td className="px-6 py-4 min-w-[220px]">
                    <p className="text-sm leading-normal break-words text-gray-600">{log.detail ?? '—'}</p>
                  </td>
                  <td className="px-6 py-4 text-xs font-mono text-gray-400 whitespace-nowrap">{log.ip ?? '—'}</td>
                  <td className="px-6 py-4 text-xs text-gray-400 whitespace-nowrap">{formatTime(log.createdAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
